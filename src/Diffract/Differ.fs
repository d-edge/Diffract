...
module Differ =
    ...
    let inline wrap<'a, 'T> (f: 'a -> 'a -> Diff option) =
        unbox<IDiffer<'T>> { new IDiffer<'a> with member _.Diff(x, y) = f x y }

    let inline simpleEquality< ^a, 'T when ^a : equality> : IDiffer<'T> =
