...
module DifferImpl =
    ...
    let private addRecursiveToCache (differ: CachedDiffer<'T>) : CachedDiffer<'T> =
        ...
        fun cache ->
            ...
            | false, _ ->
                ...
                cache.Add(
                    ty,
                    { new IDifferFactory with
                        member _.GetDiffer<'U>() =
                            { new IDiffer<'U> with
                                member _.Diff(x1, x2) = (unbox<IDiffer<'U>> !r).Diff(x1, x2) } }
                )

                let differ = differ cache
                r := differ
                differ
            | true, differFactory -> differFactory.GetDiffer<'T>()
        ...
        fun cache ->
            ...
            | false, _ ->
                cache.Add(
                    typeof<'T>,
                    { new IDifferFactory with
                        member _.GetDiffer<'U>() = unbox<IDiffer<'U>> differ }
                )
    ...
    let private wrap<'Outer, 'Inner> (f: 'Inner -> 'Inner -> Diff option) : CachedDiffer<'Outer> =
        { new IDiffer<'Inner> with
            member _.Diff(x1, x2) = f x1 x2 }
        |> unbox<IDiffer<'Outer>>
    ...
    let inline private failwith (msg: string) =
        raise (DifferConstructionFailedException(msg))
    ...
    let rec diffWith<'T> (custom: ICustomDiffer) (cache: Cache) : IDiffer<'T> =
        ...
        let (|Custom|_|) shape =
            custom.GetCustomDiffer(getCached, shape)
        ...
        | Shape.FSharpRecord (:? ShapeFSharpRecord<'T> as r) ->
            addRecursiveToCache (diffFields custom r.Fields Diff.Record) cache
        | Shape.FSharpUnion (:? ShapeFSharpUnion<'T> as u) ->
            let tagNames = u.UnionCases |> Array.map (fun c -> c.CaseInfo.Name)

            let tagDiffs =
                u.UnionCases
                |> Array.map (fun c -> diffFields custom c.Fields (fun d -> Diff.UnionField(c.CaseInfo.Name, d)) cache)
                    ...
                    member _.Visit<'Enum, 'Underlying
                        ...
                        wrap<'T, 'Enum>
                            (fun x1 x2 ->
                                ...
                                if (x1 :> obj).Equals(x2) then
                                    None
                                else
                                    Some(Diff.Value(x1, x2)))
                            cache }
                |> e.Accept
        ...
        | Shape.Equality e ->
            { new IEqualityVisitor<IDiffer<'T>> with
                member _.Visit<'Actual when 'Actual: equality>() =
                    wrap<'T, 'Actual> (fun x1 x2 -> if x1 = x2 then None else Some(Diff.Value(x1, x2))) cache }
            |> e.Accept
    ...
    and diffReadOnlyFields<'T>
        (custom: ICustomDiffer)
        (members: IShapeReadOnlyMember<'T>[])
        (wrapFieldDiffs: IReadOnlyList<FieldDiff> -> Diff)
        (cache: Cache)
        : IDiffer<'T> =
        let fields =
            members
